...
def configure_interconnect(sw, spi0=0x00, spi1=0x00, i2c0=0x00, uart0=0x00, uart1=0x00, recorder0=0x00,recorder1=0x00, pat=0x00):
  ...
  if uart1 != 0x00:
    addr = localenv["wfg_drive_uart_top_1_select_0"]["addr"]
    set_register_8bit(sw, addr, uart1)
  if recorder0 != 0x00:
...
def reenable_stim_mem_1(sw, start=0x0000, end=0x00FF):
  localenv = FPGA_Reg.registers["wfg_stim_mem_top_1"]
...
def configure_record_mem(sw, localenv, en=1, start=0x0000, end=0x1000, step=0x04, count=0):
  cnt_shift = localenv["CFG"]["CNT"]["LSB"]
  addr = localenv["CFG"]["addr"]
  set_register(sw, addr, count << cnt_shift)
...
def configure_record_mem_0(sw, en=1, start=0x0000, end=0x1fff, step=0x04, count=0):
...
def configure_record_mem_1(sw, en=1, start=0x0000, end=0x1fff, step=0x04, count=0):
...
def configure_drive_uart_0(sw, en=1, div=32, txlen=8, paritysel=0, stopsel=0, txdelay=0, shiftdir=0, timeout=0):
  ...
  timeoutshift = localenv["RX_CFG"]["TIMEOUT"]["LSB"]
  addr = localenv["RX_CFG"]["addr"]
  set_register(sw, addr, (timeout << timeoutshift))

  en_shift = localenv["CTRL"]["EN"]["LSB"]
  addr = localenv["CTRL"]["addr"]
  set_register_8bit(sw, addr, en << en_shift)

def configure_drive_uart_1(sw, en=1, div=32, txlen=8, paritysel=0, stopsel=0, txdelay=0, shiftdir=0, timeout=0):
  localenv = FPGA_Reg.registers["wfg_drive_uart_top_1"]
